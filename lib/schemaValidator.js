...
class SchemaValidator {
  ...
  constructor(schemaDir) {
    ...
    this.ajv.addKeyword('userInterfaceSchemaTarget');
    this.ajv.addKeyword('userInterfaceSchemaValidValues');
  ...
  async validateSchema(instance) {
    ...
    if (success) {
      ...
      if (!valid) {
        ...
        if (Schema == 'https://raw.githubusercontent.com/NSSAC/templated_configuration_system/main/schema/template_context.json') {
          const template = await this.load('self://'+ path.basename(instance.templateId), path.dirname(instance.__path));
          this.ajv.addSchema(template.userInterfaceSchema, template.id).validate;
          const validate = this.ajv.getSchema(template.id);
